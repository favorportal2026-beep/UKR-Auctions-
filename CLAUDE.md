...
- **Vercel + Next.js 14** — вебдодаток у `web/`, каталог/мапа/деталі/критерії.
...
## Актуалізація 2026-09-18

README та web/README.md описують поточний функціонал. Основні зміни:
- Архів: зберігаємо завершальні статуси для наявних лотів; нові закриті лоти не імпортуємо.
- Активна видача: `is_active` + незавершений дедлайн; невідомий дедлайн допускається.
- `ua_lot_matches` / `ua_rematch_lots` у Supabase — джерело істини для matching.
- Повний перерахунок транзакційний, без REST-лімітів; наявний notified зберігається.
- Каталог: 48/сторінку. Мапа читає всі сторінки, передає лише потрібні поля.
- CSV СЕТАМ автоматично вибирається через CKAN, SETAM_CSV_URL — лише override.
- Обидва джерела збираються щодня; sync_state має час успіху, статус/помилку і дату CSV.
- Prozorro refresh перевіряє відомі процедури й має окремий курсор.
- Статуси відбору/нотатки доступні на мапі та в деталях /lots/<uuid>.
- Міграція відтворює лише UA-таблиці; база спільна з UK, інші таблиці не змінювати.
- Тести: npm test; інтеграція matching — tests/rematch.sql (ROLLBACK).
